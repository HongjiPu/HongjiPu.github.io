...
| 想改什么 | 改哪个文件 |
| --- | --- |
| **Research Interests 研究图谱** | `_data/graph.yml` |
...
## 2.5 首页大图上方的 Featured 区块

`_data/featured.yml`。左边白底文字、右边配图，下方一排标签可切换，也会每 7 秒自动轮换。

```yaml
cards:
  - date: "2026年5月"
    title: "大标题，会自动带一个蓝色 › 箭头"
    excerpt: >
      标题下面那段摘要，两到四行最好看。
    url: /research/foundations/          # 点标题跳到哪
    image: /images/featured/xxx.jpg      # 右侧配图
    tab: "下方标签的短标题"               # 不写就用 title
```

- 配图放进 `images/featured/`，**建议横图、宽 1200–1600px**。卡片高度是固定的，
  图片会自动裁切填满，所以主体别贴边
- 增删卡片直接改这个列表，下方标签栏会自动跟着变
- 手机上配图会自动隐藏（优先保证文字可读），标签只显示前两个
- 浏览器窗口高度小于 780px 时整块自动隐藏，把首屏让给姓名和定位

> `images/featured/skills-placeholder.jpg` 是**临时占位图**（第三方图库照片），
> 换成你自己的图之后请删掉，避免版权问题。
> 另外三张 `.svg` 是我按站点配色生成的抽象图，可以一直用，也可以随时替换。

---

## 2.8 研究图谱

`_data/graph.yml`。首页 Research Interests 那块的点状关系图。

```yaml
nodes:
  - id: rl                    # 唯一标识，别的点靠它引用
    label: "Reinforcement Learning"
    weight: 80                # 1-100，决定点的大小和颜色深浅 = 主次
    orbit: 1                  # 0=中心  1=内圈核心方向  2=外圈具体课题
    angle: 150                # 角度，0=正右，逆时针为正。不写则本圈自动均分
    note: "鼠标悬停时显示的一句话"
    links: [marl]             # 除自动连中心外，还连到哪些点
```

- **weight 就是"主次"** —— 点越大、颜色越深。按你的实际投入自己调，中心固定 100
- **内圈会自动连到中心**，外圈必须用 `links` 指明挂在哪个方向下
- **angle 用来避免文字打架**。改完刷新看一眼，两个点角度太接近标签就会叠在一起；
  外圈相邻至少留 30°
- 颜色是一条单色阶（浅→深），不用改；加点删点都不用动代码
- 手机上图形会自动换成表格（图缩小后标签看不清），无需额外处理
